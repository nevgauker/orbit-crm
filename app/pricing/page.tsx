...
// import { getPackages } from "@/data/packages/packages";
...
function PackageCard({ pkg, isDisabled }: { pkg: Package, isDisabled: boolean }) {
    return (
        ...
        >
            ...
            <Link
                href={pkg.link}
                className={`block text-center py-2 px-4 rounded ${isDisabled
    ...
    return (
        <div className="flex flex-col items-center py-10 px-6 bg-gray-50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-5xl">
                {packages.map((pkg) => <PackageCard pkg={pkg} isDisabled={userType === pkg.userType} />)}
            </div>
        </div>
    );
}
